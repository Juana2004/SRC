...
class FormatoListaDeEspera(ABC):
    ...
    def __init__(self):
        if type(self) is FormatoListaDeEspera:
            raise TypeError(
                "FormatoListaDeEspera es una clase abstracta y no puede ser instanciada directamente."
            )
        super().__init__()
    ...
    def _formato_linea(self, linea):
        ...
        if linea.startswith("="):
            ...
            keyword in linea_lower for keyword in ["paciente", "id", "nombre", "edad"]
        ):
            return f"📋 {linea}"
        elif any(
            keyword in linea_lower for keyword in ["urgente", "crítico", "prioridad"]
        ):
            return f"🚨 {linea}"
    ...
    def count_patients(self, contenido):
        ...
        paciente_lineas = [
            l for l in lineas if not l.startswith("-") and not l.startswith("=") and l
        ]
