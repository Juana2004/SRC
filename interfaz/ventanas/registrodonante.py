...
class RegistroDonanteApp(RegistroBaseApp):
    ...
    def validate_fields(self):
        ...
        if not any(var.get() for var in self.organo_vars.values()):
            messagebox.showerror(
                "Error", "Debe seleccionar al menos un órgano para donar."
            )
            return False
        return True
    ...
    def register_donante(self):
        ...
        try:
            ...
            fecha_fallecimiento = self.widgets_hora(
                self.fecha_fallecimiento, self.hora_var, self.minuto_var
            )

            organos_seleccionados = [
                tipo.value for tipo, var in self.organo_vars.items() if var.get()
            ]
            ...
            donante = Donante(
                datos, tipo_sangre, centro, fecha_fallecimiento, organos_seleccionados
            )
