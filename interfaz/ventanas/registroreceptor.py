...
class RegistroReceptorApp(RegistroBaseApp):
    ...
    def crear_campos_receptor(self):
        ...
        organo_combo = ttk.Combobox(
            self.main_frame, textvariable=self.organo_var, width=27
        )
        ...
        ttk.Label(self.main_frame, text="Patología:").grid(
        ...
        urgencia_combo = ttk.Combobox(
            self.main_frame, textvariable=self.urgencia_var, width=27
        )
        ...
        try:
            ...
            if not centro:
                messagebox.showerror(
                    "Error", f"Centro de salud '{self.centro_var.get()}' no encontrado."
                )
                return
            ...
            if patologia_obj is None:
                messagebox.showerror(
                    "Error",
                    f"Patología '{nombre_patologia}' no encontrada para el órgano {organo}.",
                )
                return
