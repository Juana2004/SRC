...
class RegistroDonanteVivoApp:
    def __init__(self, root, incucai):
        self.root = root
        self.incucai = incucai
        self.root.title("Registro de Donantes Vivos")
        self.root.geometry("600x650")
        self.root.resizable(False, False)
        ...
        # Título
        ttk.Label(main_frame, text="Registro de Donantes Vivos", style="Header.TLabel").grid(row=0, column=0, columnspan=2, pady=(0, 20), sticky="n")
        
        # Creación de los campos
        self.create_fields(main_frame)
        
        # Botones - CORREGIDO: movido a row=10
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=10, column=0, columnspan=2, pady=20)
        ttk.Button(button_frame, text="Registrar", command=self.register_donante_vivo, style="Accent.TButton").grid(row=0, column=0, padx=10)
        ttk.Button(button_frame, text="Limpiar", command=self.clear_fields).grid(row=0, column=1, padx=10)
    ...
    def create_fields(self, parent):
        ...
        self.sangre_var = tk.StringVar()
        sangre_combo = ttk.Combobox(parent, textvariable=self.sangre_var, width=27)
        # Importar TipoSangre dynamically here or use the values directly from the existing interface code
        from tipos.tipo_sangre import TipoSangre
        sangre_combo['values'] = [tipo.value for tipo in TipoSangre]
        sangre_combo.grid(row=6, column=1, sticky=tk.W, pady=5)
        sangre_combo.current(0)
        ...
        ttk.Label(parent, text="Información de Ablación:").grid(row=8, column=0, sticky=tk.W, pady=5)
        ...
        # Crear variables y checkbuttons para cada tipo de órgano vivo disponible
        self.organ_vars = {}
        
        # Obtener la lista de órganos que pueden ser donados en vida
        from tipos.tipo_organos_vivos import TipoOrganoVivo
        
        # Organizar los checkbuttons en columnas
        organs_per_column = 3
        organos_vivos = [tipo.value for tipo in TipoOrganoVivo]
        for i, organo in enumerate(organos_vivos):
            # Calcular posición en la grid
            col = i // organs_per_column
            row = i % organs_per_column
