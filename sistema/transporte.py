...
class Transporte:
    ...
    def asignar_vehiculo(
        self, donante: Paciente, receptor: Receptor, vehiculos: list[Vehiculo]
    ) -> tuple[bool, float]:
        """
        ...
        vehiculo_optimo = self._seleccionar_mejor_vehiculo(
            vehiculos, centro_donante, centro_receptor
        )
        tiempo_total = vehiculo_optimo.ejecutar_transporte(
            centro_donante, centro_receptor
        )

        return True, tiempo_total

    def obtener_vehiculos_por_ubicacion(
        self, origen: CentroDeSalud, destino: CentroDeSalud
    ) -> list[Vehiculo]:
        """
        ...
        return [
            v
            for v in (
                self.incucai.aviones
                + self.incucai.helicopteros
                + self.incucai.vehiculos_terrestres
            )
            if v.puede_realizar_transporte(origen, destino)
        ]

    def _seleccionar_mejor_vehiculo(
        self, vehiculos: list[Vehiculo], origen: CentroDeSalud, destino: CentroDeSalud
    ) -> Vehiculo:
        """
        Metodo privado
        ...
        return min(
            vehiculos,
            key=lambda v: (
                v.calcular_tiempo_total_mision(origen, destino)
                if hasattr(v, "calcular_tiempo_total_mision")
                else float("inf")
            ),
        )

    def _informar_distancia(
        self, origen: CentroDeSalud, destino: CentroDeSalud
    ) -> float:
        """
        Metodo privado
