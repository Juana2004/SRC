...
class Validaciones:
    ...
    def validar_cedula_unica(self, persona: object):
        if (
            persona
            in self.incucai.cirujanos_especializados + self.incucai.cirujanos_generales
        ):
            raise ErrorCedulaRepetido(persona.nombre)

    def validar_datos(
        self, objeto: object, campos_esperados: dict, opcionales: list = []
    ):
